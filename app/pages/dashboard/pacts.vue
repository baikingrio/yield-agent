...
const pactLogs = ref<import('#shared/types/app').LogEntry[]>([])
...
async function loadPactLogs(pactId: string | null) {
  ...
  try {
    pactLogs.value = await $fetch<import('#shared/types/app').LogEntry[]>('/api/logs', {
...
<template>
  <div class="mx-auto max-w-6xl">
    ...
    <div v-else-if="filteredPacts.length === 0" class="rounded-lg border border-dashed border-hairline px-5 py-12 text-center">
      ...
      <NuxtLink :to="DASHBOARD_CREATE_STRATEGY" class="mt-4 inline-block text-sm font-medium text-primary hover:underline">
        创建策略
      </NuxtLink>
    </div>
