...
- `AGENT_WALLET_MAIN_NODE_ID`: TSS Node ID used when creating MPC wallets.
...
Vercel is suitable for the Nuxt frontend and stateless API routes, but it is not suitable for long-running TSS Node processes. A production CAW runtime should run TSS Node on a local dev machine, secure VPS, or dedicated worker environment.
