import process from 'node:process';globalThis._importMeta_=globalThis._importMeta_||{url:"file:///_entry.js",env:process.env};import { defineComponent, shallowRef, getCurrentInstance, provide, cloneVNode, h, createElementBlock, ref, inject, Suspense, hasInjectionContext, Fragment, computed, shallowReactive, defineAsyncComponent, unref, createApp, onErrorCaptured, onServerPrefetch, createVNode, resolveDynamicComponent, reactive, effectScope, mergeProps, getCurrentScope, toRef, withCtx, nextTick, isReadonly, isRef, toValue, toRaw, useSSRContext, isShallow, isReactive } from 'vue';
import { p as parseURL, f as encodePath, n as decodePath, o as hasProtocol, q as isScriptProtocol, l as joinURL, w as withQuery, t as sanitizeStatusCode, v as getContext, $ as $fetch$1, x as defu, c as createError$1, y as executeAsync, z as hash } from '../nitro/nitro.mjs';
...
const unhead_FDsMqOpN9wtA6yjCbHxX_fmC5pAg0Fw9rT2MdN_mZ0M = /* @__PURE__ */ defineNuxtPlugin({
  name: "nuxt:head",
  enforce: "pre",
  setup(nuxtApp) {
...
const _routes = [
  {
    ...
    component: () => import('./create-strategy-Uw6dU40s.mjs')
  },
  {
    name: __nuxt_page_meta$1?.name,
    path: "/dashboard",
    meta: __nuxt_page_meta$1 || {},
    component: () => import('./dashboard-Br1QtETT.mjs'),
    children: [
      {
        name: "dashboard-create-strategy",
        path: "create-strategy",
        component: () => import('./create-strategy-B2h8s3Ko.mjs')
      },
      {
        name: "dashboard-history",
        path: "history",
        component: () => import('./history-CeTLbctG.mjs')
      },
      {
        name: "dashboard-pacts",
        path: "pacts",
        component: () => import('./pacts-BipCwvfB.mjs')
      },
      {
        name: "dashboard-settings",
        path: "settings",
        component: () => import('./settings-PIYEdStT.mjs')
      },
      {
        name: "dashboard",
        path: "",
        component: () => import('./index-BqqKA8Gj.mjs')
      }
    ]
  },
  {
    name: "history",
    path: "/history",
    component: () => import('./history-CERl1R3t.mjs')
  },
  {
    name: "pacts",
    path: "/pacts",
    component: () => import('./pacts-DpUbDTdP.mjs')
  },
  {
    name: "settings",
    path: "/settings",
    component: () => import('./settings-V7vC4aNA.mjs')
  },
  {
    name: "wallet",
    path: "/wallet",
    component: () => import('./wallet-Nca92gNM.mjs')
  },
  {
    name: "index",
    path: "/",
    meta: __nuxt_page_meta || {},
    component: () => import('./index-DMz2txXj.mjs')
  }
];
...
function extractApiErrorMessage(err, fallback = "请求失败，请稍后重试") {
...
const useAppStore = defineStore("app", () => {
  ...
  const pacts = ref([]);
  const selectedPact = ref(null);
  const logs = ref([]);
  const yieldSeries = ref(null);
  const yieldRange = ref("7d");
  const settings = ref(null);
  const preparation = ref(null);
  const agentBootstrap = ref(null);
  const cawReadiness = ref(null);
  const cawOnboardStatus = ref(null);
  const strategyAgentReadiness = ref(null);
  const strategyAgentPing = ref(null);
  const loading = ref(false);
  const error = ref(null);
  async function fetchWallet() {
    try {
      wallet.value = await $fetch("/api/wallet");
    } catch (e) {
  ...
  async function fetchStrategies() {
    try {
      strategies.value = await $fetch("/api/strategies");
    } catch (e) {
      error.value = apiErrorMessage(e);
      throw e;
    }
  }
  async function fetchPacts(status, options) {
    try {
      const query = {
        ...status ? { status } : {},
        ...options?.sync ? { sync: "true" } : {}
      };
      pacts.value = await $fetch("/api/pacts", { query });
    } catch (e) {
      error.value = apiErrorMessage(e);
      throw e;
    }
  }
  async function fetchPact(id, options) {
    try {
      selectedPact.value = await $fetch(`/api/pacts/${id}`, {
        query: options?.sync ? { sync: "true" } : void 0
      });
  ...
  async function fetchLogs(params) {
    try {
      logs.value = await $fetch("/api/logs", { query: params });
    } catch (e) {
      error.value = apiErrorMessage(e);
      throw e;
    }
  }
  async function fetchYieldSeries(range, options) {
    const r = range ?? yieldRange.value;
    try {
      yieldSeries.value = await $fetch("/api/yield-series", {
        query: {
          range: r,
          ...options?.sync ? { sync: "true" } : {}
        }
  ...
  async function fetchCawReadiness() {
    try {
      cawReadiness.value = await $fetch("/api/caw/readiness");
  ...
  async function fetchStrategyAgentReadiness() {
    try {
      strategyAgentReadiness.value = await $fetch("/api/strategy-agent/readiness");
  ...
  async function provisionCawAgent(name) {
    try {
      const result = await $fetch("/api/caw/provision", {
        method: "POST",
        body: { name }
      });
  ...
  async function fetchCawOnboardStatus() {
    try {
      cawOnboardStatus.value = await $fetch("/api/caw/onboard/status");
  ...
  async function startCawOnboard(agentName, wait = false) {
    try {
      cawOnboardStatus.value = await $fetch("/api/caw/onboard/start", {
        method: "POST",
        body: { agentName, wait }
      });
  ...
  async function continueCawOnboard(sessionId, answers, wait = false) {
    try {
      cawOnboardStatus.value = await $fetch("/api/caw/onboard/continue", {
        method: "POST",
  ...
  async function pingStrategyAgent() {
    try {
      strategyAgentPing.value = await $fetch("/api/strategy-agent/ping", {
        method: "POST"
      });
  ...
  async function approvePact(id) {
    const pact = await $fetch(`/api/pacts/${id}/approve`, { method: "POST" });
  ...
  async function terminatePact(id) {
    const pact = await $fetch(`/api/pacts/${id}/terminate`, { method: "POST" });
    const idx = pacts.value.findIndex((p) => p.id === id);
    if (idx >= 0) pacts.value[idx] = pact;
    if (selectedPact.value?.id === id) selectedPact.value = pact;
  ...
  async function createStrategy(payload) {
    const result = await $fetch("/api/strategies", {
      method: "POST",
      body: payload
    });
  ...
  async function executePact(id) {
    const result = await $fetch(`/api/pacts/${id}/execute`, {
      method: "POST"
    });
    await fetchPact(id);
  ...
  async function simulatePactDenial(id) {
    const result = await $fetch(`/api/pacts/${id}/simulate-denial`, {
      method: "POST"
    });
    await fetchLogs({ limit: 10, pactId: id });
    return result;
  }
  async function redeemPact(id) {
    const result = await $fetch(`/api/pacts/${id}/redeem`, {
      method: "POST"
    });
    await Promise.all([
      fetchPact(id),
  ...
  async function fetchPactPosition(id) {
    return $fetch(`/api/pacts/${id}/position`);
  }
  async function parseStrategyText(text, limits) {
    return $fetch("/api/strategy-agent/parse", {
      method: "POST",
      body: { text, limits }
    });
  }
  async function fetchPreparation() {
    try {
      preparation.value = await $fetch("/api/wallet/preparation");
  ...
  async function connectEoa(address, label) {
    preparation.value = await $fetch("/api/wallet/preparation/connect-eoa", {
      method: "POST",
      body: { address, label }
    });
  ...
  async function disconnectEoa() {
    preparation.value = await $fetch("/api/wallet/preparation/disconnect-eoa", {
      method: "POST"
    });
    wallet.value = await $fetch("/api/wallet");
    return preparation.value;
  }
  function applyAgentBootstrapResponse(response) {
    preparation.value = response.preparation;
    agentBootstrap.value = response.bootstrap ?? response.preparation.agentBootstrap ?? null;
    return response;
  }
  async function createAgentWallet() {
    const response = await $fetch("/api/wallet/preparation/create-agent", {
      method: "POST"
    });
  ...
  async function pollAgentWalletStatus() {
    const response = await $fetch("/api/wallet/preparation/agent-status");
    applyAgentBootstrapResponse(response);
    return response;
  }
  async function importAgentWalletFromCli() {
    preparation.value = await $fetch("/api/wallet/preparation/import-agent", {
      method: "POST"
    });
  ...
  async function depositToAgentWallet(amountUsdc, txHash) {
    preparation.value = await $fetch("/api/wallet/preparation/deposit", {
      method: "POST",
  ...
  async function fetchDepositInfo(amountUsdc) {
    return $fetch("/api/wallet/preparation/deposit-info", {
      query: { amountUsdc }
    });
  }
  async function fetchAgentGasStatus() {
    return $fetch("/api/wallet/preparation/gas-status");
  }
  async function resetPreparation() {
    preparation.value = await $fetch("/api/wallet/preparation/reset", {
      method: "POST"
    });
    await fetchWallet();
    return preparation.value;
...
function useWalletConnectStub() {
  ...
  return {
    ...
    expectedNetwork: computed(() => "base-sepolia"),
...
const require_45wallet_45global = /* @__PURE__ */ defineNuxtRouteMiddleware(async (to) => {
  let __temp, __restore;
  ...
  if (!store.preparation) {
    try {
      ;
      [__temp, __restore] = executeAsync(() => store.fetchPreparation()), await __temp, __restore();
      ;
    } catch {
...
const revive_payload_server_6C__4QMc_B8BnFqm0Ye5Dxp2tmOT_FSQYntnDQCnj3U = /* @__PURE__ */ defineNuxtPlugin({
...
const plugins = [
  payloadPlugin,
  unhead_FDsMqOpN9wtA6yjCbHxX_fmC5pAg0Fw9rT2MdN_mZ0M,
  plugin$1,
  revive_payload_server_6C__4QMc_B8BnFqm0Ye5Dxp2tmOT_FSQYntnDQCnj3U,
  plugin,
  components_plugin_4kY4pyzJIYX99vmMAAIorFf3CnAaptHitJgf7JxiED8
];
const layouts = {
  dashboard: defineAsyncComponent(() => import('./dashboard-CLQ-_WJY.mjs').then((m) => m.default || m)),
  default: defineAsyncComponent(() => import('./default-BtIOX_zn.mjs').then((m) => m.default || m))
};
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    const stack = void 0;
    const _Error404 = defineAsyncComponent(() => import('./error-404-C_z_qHvo.mjs'));
    const _Error = defineAsyncComponent(() => import('./error-500-D3W7Knx-.mjs'));
...
_sfc_main$1.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("../node_modules/.pnpm/nuxt@4.4.6_@babel+plugin-syntax-jsx@7.29.7_@babel+core@7.29.7__@babel+plugin-syntax-typ_50a001b47851142f2ab873843af3b89d/node_modules/nuxt/dist/app/components/nuxt-error-page.vue");
...
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("../node_modules/.pnpm/nuxt@4.4.6_@babel+plugin-syntax-jsx@7.29.7_@babel+core@7.29.7__@babel+plugin-syntax-typ_50a001b47851142f2ab873843af3b89d/node_modules/nuxt/dist/app/components/nuxt-root.vue");
...
export { _export_sfc as _, __nuxt_component_0 as a, useAppStore as b, __nuxt_component_1 as c, useRouter as d, entry_default as default, extractApiErrorMessage as e, useWalletConnect as f, useNuxtApp as g, encodeRoutePath as h, useRuntimeConfig as i, nuxtLinkDefaults as j, navigateTo as n, resolveRouteObject as r, useRoute as u };
